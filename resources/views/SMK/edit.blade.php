...
<form action="{{ route('SMK.updateprofile', $data->id) }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            ...
            <br>
                <div class="col-md-8 mt-1 mx-auto text-center bg-primary text-white float-center border border-info"><h4><strong><b>{{$data->nama}}</b> MELAKUKAN EDIT AKUN</strong></div>
                    ...
                    <label for="">Password Lama atau membuat Password Baru</label>
                        ...
                        <a href="{{url('DashboardSMK')}}" class="btn btn-primary pull-right">Kembali ke Halaman Utama</a>
                        @endforeach
